...
void utilWriteInt(gzFile file, int i)
{
  utilGzWrite(file, &i, sizeof(int));
}

int utilReadInt(gzFile file)
{
  int i = 0;
  utilGzRead(file, &i, sizeof(int));
  return i;
}
...
{
  /*utilGzWriteFunc = (int (ZEXPORT *)(void *,void * const, unsigned int))gzwrite; ichfly todo
  utilGzReadFunc = gzread;
  utilGzCloseFunc = gzclose;

  return gzopen(file, mode);*/
  	//fatInitDefault(); //no ichfly nito is faster
	return fopen(file, mode);
  
}

gzFile utilMemGzOpen(char *memory, int available, char *mode) //ichfly todo
{
/*  utilGzWriteFunc = memgzwrite;
  ...
  return fopen(memory,mode); //ichfly hope that works 
}

int utilGzWrite(gzFile file, const voidp buffer, unsigned int len)
{
  return fwrite(buffer,1,len,(FILE*)file);//utilGzWriteFunc(file, buffer, len); ichfly todo
}

int utilGzRead(gzFile file, voidp buffer, unsigned int len)
{
  return fread(buffer,1,len,(FILE*)file);//utilGzReadFunc(file, buffer, len);
}

int utilGzClose(gzFile file)
{
  return fclose((FILE*)file);//utilGzCloseFunc(file);
}

long utilGzMemTell(gzFile file)
{
/*  return memtell(file);*/ //ichfly
...
void utilUpdateSystemColorMaps()
{
      for(int i = 0; i < 0x10000; i++) {
        systemColorMap16[i] = ((i & 0x1f) << systemRedShift) |
          ...
          (((i & 0x7c00) >> 10) << systemBlueShift)  | BIT(15);
      }
}
