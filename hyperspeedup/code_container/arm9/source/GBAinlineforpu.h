...
iprintf("r8 %02x\n",address);
  ...
  case 4:
    ...
    if((address < 0x4000400) && ioReadable[address & 0x3ff])
      return ioMem[(address & 0x3ff) + dsuncashedoffset];
    else goto unreadable;
  case 8:
  case 9:
  case 10:
  ...
  cpuNextEvent = cpuTotalTicks;
...
#endif
	  #ifdef arm9advsound
		  REG_IPC_FIFO_TXs((address & 0x3FF) | 0x40000000 | (b << 12));
		  //REG_IPC_FIFO_TX = b;
		#endif
	break;
      default:
	...
	if(address & 1)
	{
	  CPUUpdateRegister(address & 0x3fe,
			    ((READ16LE(((u16 *)&ioMem[(address & 0x3fe) + dsuncashedoffset])))
			     & 0x00FF) |
			    b<<8);
	  ...
	  CPUUpdateRegister(address & 0x3fe,
			    ((READ16LE(((u16 *)&ioMem[(address & 0x3fe) + dsuncashedoffset])) & 0xFF00) | b));
      }
      break;
    } else goto unwritable;
    break;   
