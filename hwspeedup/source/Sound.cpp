  ...
  if(systemSoundOn) {
    ...
    if(2*soundBufferIndex >= soundBufferLen) {
      if(systemSoundOn) {
        ...
        //systemWriteDataToSoundBuffer(); //ichfly
      }
      soundIndex = 0;
      soundBufferIndex = 0;
    }
...
{
  utilWriteData(file, soundSaveStruct);
  utilWriteData(file, soundSaveStructV2);
  
  utilGzWrite(file, &soundQuality, sizeof(int));
}

void soundReadGame(gzFile file, int version)
{
  utilReadData(file, soundSaveStruct);
  if(version >= SAVE_GAME_VERSION_3) {
    utilReadData(file, soundSaveStructV2);
  } else {
  ...
  int quality = 1;
  utilGzRead(file, &quality, sizeof(int));
